		...
		<tbody>
			<c:forEach items="${sessionScope.infoList2}" var="info">
				<form method="post">
					...
					<!-- 是否提交不应该交给管理员修改 -->
					<td>${info.sftj}</td>
					<td><input type="text" value=${info.sflx} name="text22"/></td>
					<td><input type="submit" formaction="shengheupdate.action" value="修改" name="update"/></td>
					<td><input type="submit" formaction="shenghedelete.action" value="删除" name="delete"/></td>
				</tr>
				</form>
			</c:forEach>
		</tbody>
